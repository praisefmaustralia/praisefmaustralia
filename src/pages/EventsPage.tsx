...
const GOSPEL_KEYWORDS = [
  // Internacionais que costumam fazer shows na Austrália
  'Hillsong',
  'Bethel Music',
  'Chris Tomlin',
  'Lauren Daigle',
  ...
  'Rend Collective',
  'Phil Wickham',
  // Artistas e ministérios australianos
  'Hillsong United',
  'Hillsong Young & Free',
  'Planetshakers',
  'CityAlight',
...
function formatDate(localDate: string) {
  ...
  return {
    ...
    full: d.toLocaleDateString('en-AU', { month: 'long', day: 'numeric', year: 'numeric' }),
  };
}
          ...
          <div>
            ...
            {event.dates.start.localTime && (
              <p className="text-[11px] text-gray-400 mt-0.5">
                {new Date(`2000-01-01T${event.dates.start.localTime}`).toLocaleTimeString('en-AU', { hour: 'numeric', minute: '2-digit' })}
              </p>
            )}
          </div>
        </div>
...
const EventsPage: React.FC = () => {
  ...
  const load = useCallback(async () => {
    if (!TM_API_KEY) { setError('missing_key'); setLoading(false); return; }
  ...
  const cities = new Set(events.map(e => getVenue(e)?.city?.name).filter(Boolean));
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tight mb-3">Gospel Events Australia</h1>
          <p className="text-orange-100 text-lg max-w-xl mx-auto">Real upcoming concerts & worship nights across Australia — powered by Ticketmaster.</p>
          {!loading && events.length > 0 && (
            <div className="flex justify-center gap-8 mt-8">
              {[{ v: events.length, l: 'Events' }, { v: GOSPEL_KEYWORDS.length, l: 'Artists' }, { v: cities.size, l: 'Cities' }].map(({ v, l }) => (
                <React.Fragment key={l}>
                  <div><p className="text-3xl font-black">{v}</p><p className="text-orange-200 text-xs uppercase tracking-widest mt-0.5">{l}</p></div>
                  {l !== 'Cities' && <div className="w-px bg-white/20" />}
                </React.Fragment>
              ))}
            </div>
          )}
