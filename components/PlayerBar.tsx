...
export const PlayerBar: React.FC<PlayerBarProps> = ({
  ...
  const displayTitle = liveMetadata?.title || current.title;
  const displayArtist = liveMetadata?.artist || current.host;
  const displayImage =
    liveMetadata?.artwork || current.image || `https://picsum.photos/seed/${current.id}/100`;
